...
- **다양한 도서관 유형**: 공공도서관, 24시간 스마트도서관, 교육청도서관까지 모두 포함
...
| 구분 | 책바다 | 리브로피아 | 서울시 통합검색 | 책크 (본 서비스) |
|---|---|---|---|---|
...
- **해결**: 책크에서 제목과 저자를 함께 검색하여 집 근처 3개 도서관 중 2km 거리에 있는 도서관에 대출 가능한 책이 있음을 확인하고 퇴근길에 방문
...
- **해결**: 책크에서 원하는 책들을 차례로 검색하여 한 도서관에서 모두 빌릴 수 있는지 확인
...
#### 검색 방식
**복합 검색 지원**: 제목, 저자, 출판사를 조합하여 검색 정확도 향상

- **제목**: 책 제목 키워드 입력 (선택사항)
- **저자**: 저자명 입력 (선택사항)
- **출판사**: 출판사명 입력 (선택사항)

사용자는 3가지 필드 중 **최소 1개 이상** 입력하여 검색할 수 있습니다. 여러 필드를 함께 입력하면 더 정확한 검색 결과를 얻을 수 있습니다.

**검색 예시**:
- 제목만 입력: "멋진 신세계" → 모든 버전의 "멋진 신세계" 검색
- 제목 + 저자: "멋진 신세계" + "올더스 헉슬리" → 특정 저자의 책만 검색
- 저자 + 출판사: "무라카미 하루키" + "문학동네" → 특정 출판사의 특정 저자 책만 검색
...
flowchart TD
    ...
    SearchPage --> InputSearch[검색 정보 입력<br/>제목, 저자, 출판사<br/>1개 이상 필수]
...
- **검색 영역** (중앙 배치)
  ...
  - **저자 입력창**
    - 플레이스홀더: "저자명을 입력하세요 (선택)"
  - **출판사 입력창**
    - 플레이스홀더: "출판사명을 입력하세요 (선택)"
  - 안내 텍스트: "최소 1개 이상 입력해주세요"
  - 검색 버튼
...
#### 3.1 기본 지도 기능
- **도서관 위치 시각화**: 검색 결과의 모든 도서관을 지도에 마커로 표시
  - 도서관 유형별 다른 색상/아이콘 (공공도서관/스마트도서관/교육청도서관)
  - 대출 가능 여부에 따른 마커 색상 구분 (녹색: 대출 가능, 빨강: 대출 중)
- **현재 위치 표시**: 사용자의 현재 위치를 지도에 표시
- **인터랙티브 지도**:
  - 마커 클릭 시 도서관 정보 팝업 (도서관명, 거리, 대출 가능 여부)
  - 지도 확대/축소, 드래그로 주변 탐색

#### 3.2 지도 API 선택
- **카카오맵 API** (1순위 추천):
  - 한국 지도 데이터 정확도 높음
  - 무료 사용량 충분 (일 300,000건)
  - 한글 문서 및 샘플 코드 풍부
  - 거리 계산 API 제공
- **네이버 지도 API** (대안):
  - 국내 최고 수준의 지도 정확도
...
#### 3.3 고급 위치 기능
- **길찾기 연동**:
  - "길찾기" 버튼 클릭 시 카카오맵/네이버 지도 앱 실행
  - 도보/대중교통/자동차 경로 안내
  - 예상 소요 시간 표시
- **반경 검색 필터**:
  - 사용자 위치 기준 반경 1km/3km/5km 내 도서관 필터링
  - 지도 상에 반경 원 표시
- **지도/리스트 뷰 전환**:
  - 토글 버튼으로 지도 뷰 ↔ 리스트 뷰 전환
  - 지도에서 선택한 도서관을 리스트에서 하이라이트

#### 3.4 기술 스택
- **프론트엔드**: 카카오맵 JavaScript API v3
- **기능 구현**:
  - 지도 객체 생성 및 중심 좌표 설정
  - 마커 및 커스텀 오버레이 생성
  - 거리 계산 API를 통한 실시간 거리 측정
  - 모바일 환경에서 외부 지도 앱 연동 (Deep Link)
...
- v1.5 (2026-01-21): 향후 확장 계획에 지도 기반 위치 서비스 추가 (Phase 3: 카카오맵 API 활용, 길찾기 연동, 반경 검색)
- v1.6 (2026-01-21): 서비스명 확정 및 검색 방식 개선 (책크로 명명, 복합 검색 지원으로 검색 정확도 향상)
